...
int send_packet(int fd,int m_fd,int cs);
...
int main(int argc, char *argv[])
{
	...
	while(1){
			 ...
			 */
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			ret=send_packet(fd0,m_fd,0);

			printf("observe tx:\n");
			uint8_t obs0[]={READ_REG+OBSERVE_TX,0};
			uint8_t obsTmp0[2]={0, };
			SPI_Transfer(fd0,obs0,obsTmp0,2);
			
			printf("read status:\n");
			uint8_t rdsta0[]={READ_REG+STATUS,0};
			uint8_t rdstaTmp0[2]={0, };
			ret=SPI_Transfer(fd0,rdsta0,rdstaTmp0,2);

			printf("write status:\n");
			uint8_t sta0[]={WRITE_REG+STATUS,0xFF};
			uint8_t staTmp0[2]={0, };
			ret=SPI_Transfer(fd0,sta0,staTmp0,2);

			uint8_t tflush0[]={FLUSH_TX,0xFF};
			uint8_t tflushTmp0[2]={0, };
			SPI_Transfer(fd0,tflush0,tflushTmp0,2);
			...
			ret=send_packet(fd1,m_fd,1);

			printf("observe tx:\n");
			uint8_t obs1[]={READ_REG+OBSERVE_TX,0};
			uint8_t obsTmp1[2]={0, };
			SPI_Transfer(fd1,obs1,obsTmp1,2);
			
			printf("read status:\n");
			uint8_t rdsta1[]={READ_REG+STATUS,0};
			uint8_t rdstaTmp1[2]={0, };
			ret=SPI_Transfer(fd1,rdsta1,rdstaTmp1,2);

			printf("write status:\n");
			uint8_t sta1[]={WRITE_REG+STATUS,0xFF};
			uint8_t staTmp1[2]={0, };
			ret=SPI_Transfer(fd1,sta1,staTmp1,2);

			uint8_t tflush1[]={FLUSH_TX,0xFF};
			uint8_t tflushTmp1[2]={0, };
			SPI_Transfer(fd1,tflush1,tflushTmp1,2);
...
int send_packet(int fd,int m_fd,int cs){
	...
	if(cs==0){
		...
		uint8_t wr0Tmp[33]={0, };
		ret=SPI_Transfer(fd,wr0,wr0Tmp,33);
	}else if(cs==1){
		printf("spi 1 发送数据\n");
		...
		uint8_t wr1Tmp[33]={0, };
		ret=SPI_Transfer(fd,wr1,wr1Tmp,33);
		printf("是否发送成功？%d\n", ret);
	}
	usleep(10);
